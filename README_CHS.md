...
* 🌐 智能网页浏览 —— Spydra 可自主浏览互联网：搜索、阅读、提取信息、填写网页表单，全程免手动。
...
> *你能搜索 spydra 项目，了解需要哪些技能，然后打开 CV_candidates.zip 并告诉我哪些最匹配该项目吗？*
...
- **REDIS_BASE_URL**: 保持不变 
- **WORK_DIR**: 本地工作目录路径。Spydra 可读取和操作这些文件。
...
*服务启动故障排除：* 如果这些脚本失败，请确保 Docker Engine 正在运行并且 Docker Compose（V2，`docker compose`）已正确安装。检查终端输出中的错误消息。请参阅 [FAQ：帮助！运行 Spydra 或其脚本时出现错误。](#faq-故障排除)
...
*   **`[MAIN]` 部分：**
    ...
    *   `work_dir`: **关键：** Spydra 将读取/写入文件的目录。**确保此路径在您的系统上有效且可访问。**
...
| `server`                      | `False`    | 连接到在另一台机器上运行的 Spydra 自托管 LLM 服务器。 | [在自己的服务器上运行 LLM 的设置](#在自己的服务器上运行-llm-的设置) |
...
**问：当我有 Manus 时，为什么应该使用 Spydra？**

与 Manus 不同，Spydra 优先考虑独立于外部系统，给您更多控制、隐私和避免 API 成本。

**问：谁是这个项目的幕后推手？**

这个项目是由我创建的，还有两个朋友作为维护者和 GitHub 上开源社区的贡献者。我们只是一群充满热情的个人，不是初创公司，也不隶属于任何组织。

X 上除了我的个人账户（https://x.com/Ahmed Arsalan）之外的任何 Spydra 账户都是冒充的。

## 贡献

我们正在寻找开发人员来改进 Spydra！查看开放的问题或讨论。

[贡献指南](./docs/CONTRIBUTING.md)

## 赞助商：

想要通过航班搜索、旅行规划或抢购最佳购物优惠等功能来提升 Spydra 的能力？考虑使用 SerpApi 制作自定义工具，以解锁更多 Jarvis 般的功能。使用 SerpApi，您可以为专业任务加速您的代理，同时保持完全控制。
...
## 维护者：
 ...
 > [Spyder Solutions](https://github.com/Spyder Solutions) | 巴黎时间 
